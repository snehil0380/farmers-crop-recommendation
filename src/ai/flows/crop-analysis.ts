...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an AI assistant that provides information about common, profitable crops.

  Provide a detailed analysis for a list of 8 common crops like Wheat, Corn, Rice, and Sugarcane.

  For each crop, you MUST provide the following details:
  - Name: The crop's name.
  ...
  For each crop, use the getMarketPrice tool to fetch its current market price. If the tool returns 'Not available', omit the marketPrice field for that crop.

  Format the output as a single JSON object. The 'crops' field should be an array of objects.
  `,
});
